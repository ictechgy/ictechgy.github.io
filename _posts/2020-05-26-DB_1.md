...
위의 두개는 동등조인이다. 아래는 자연조인 (차수의 개수에 차이가 난다.)
...
이 경우에 특별한 조건이 없으므로 처음부터 조인을 하고 검색해도 된다.
...
위의 SQL문장은 열의 순서가 맞지 않아 오류가 난 것이다. - domain이 다름
...
View 테이블은 창문일 뿐, 데이터를 가지고 있지 않다. 
...
### 72. 급여가 1000 이상이면서 3000 이하인 사번, 사원명, 급여, 부서번호를 보이는 emp_view2라는 view를 생성하시오.
1)
select empno, ename, sal, deptno
from emp
where sal between 1000 and 3000
...
create table b (
	...
	constraint b_pk primary key(______));

create table a (
	a1 char(3) ______,
	a2 varchar2(10) null,
	b1 ______ ______,
	constraint a_pk primary key (_______),
	constraint a_fk foreign key (_______) references _______(_______)
		on delete cascade
);
...
자식테이블 생성시 부모테이블에 대하여 자기자신이 cascade로 연쇄삭제가 되도록 생성
...
insert into a values ('a01', null, 10);
insert into a values ('a02', 'aaa', 10);
...
create table 학생(
	학번 number(4) ___________,
	학생명 varchar2(20) not null,
	학년 number(1) null,
	학과번호 ________ __________,
	constraint 학생_pk primary key(학번),
	constraint 학생_fk foreign key(_________) references _______(________________),
	constraint 학년_ck check (_________________)
);
** 학년은 1에서 4 사이 값만 와야한다. 
...
문제)  성공/실패 쓰기 (실패시 이유 설명하시오)
insert into 학과 values('a1', '컴공'); - 성공


insert into 학과 values('b1', '보안'); - 성공

insert into 학과 values(null, '멀티'); - 실패. 기본키에는 null이 들어갈 수 없다. 개체무결성에 위배


insert into 학과 values('a1', '게임'); - 실패. 이미 a1이라는 기본키값은 들어가있다. 개체무결성에 위배


insert into 학생 (학번, 학생명,학과번호)
		values   (1000,'홍',null); - 실패할 것이라고 생각한다. 학년에 1~4 사이의 값이 들어가야하는데 해당 값을 넣지 않고 있다. 도메인 범위를 만족하지 않는다.
-> 삽입이 가능하다. 학년부분에 null값이 정상적으로 들어간다. 값이 들어온다면 해당 값을 체크하겠다는 것임. null가능 여부는 create에서 정하는 것이다.


insert into 학생 (학번, 학생명,학과번호)
		values   (2000,'kim','a1'); - 성공

insert into 학생 (학번, 학생명,학과번호)
		values   (3000,'park','c1'); - 실패. 부모테이블에 c1에 해당하는 기본키를 가진 레코드가 존재하지 않는다. 참조무결성에 위배된다.

insert into 학생 (학번, 학생명,학과번호)
		values   (1000,'lee','a1'); - 실패. 기본키인 학번1000번을 가지고 있는 레코드가 이미 존재한다. 개체무결성에 위배

insert into 학생 (학번, 학생명,학과번호)
		values   (null,'kang','b1'); - 실패. 기본키에 null을 넣으려고 하고 있다. 개체무결성에 위배

insert into 학생 (학번, 학생명,학년,학과번호)
		values   (4000,'jung',1,'a1'); - 성공
	
insert into 학생 (학번, 학생명,학년,학과번호)
		values   (5000,'choi',5,'b1'); - 실패. 학년에 1~4사이의 값이 아닌 값이 들어가려 한다. domain constraint 에 위배
...
update 학생
set 학과번호 ='c1'
where 학번 = 2000; - 실패. 바꾸려는 학과번호 c1은 부모테이블에 존재하지 않는다. 참조무결성에 위배된다.

update 학생
set 학번 = null
where 학번 = 1000; - 실패. 학번은 기본키인데 null이 들어갈 수없다. 개체무결성에 위배

update 학생
set 학년 = 6
where 학번 = 4000; - 실패. 학년에는 1~5 사이의 값만 들어갈 수 있다. check constraint 위배
...
update 학과
set 학과번호 = 'a2'
where 학과번호='a1'; - 실패. 자식테이블에서 a1에 해당하는 기본키를 외래키로서 참조하고 있다. 참조무결성에 위배

update 학과
set 학과번호 = 'b2'
where 학과번호='b1';  - 성공. 참조하고 있는 자식이 없음

delete from 학생
where 학번=4000; - 성공

delete from 학과
where 학과번호='a1'; - 실패. a1에 해당하는 기본키를 참조하고 있는 자식이 존재한다. 참조무결성에 위배

delete from 학과
where 학과번호='b2'; - 성공. 참조하는 자식이 없음

drop table  학과; - 실패. 학과에 존재하는  기본키를 참조하는 자식테이블이 존재한다. 참조무결성에 위배
