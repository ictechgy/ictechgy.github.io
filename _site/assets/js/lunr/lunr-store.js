var store = [{
        ...
        "excerpt":"flatMap에서 error가 담긴 Observable이 반환되는 경우 여태까지 나는 ‘flatMap을 통해 반환되는 Observable이 기존 시퀀스에는 전혀 영향을 미치지 않는다.’라고 생각해왔다. (왜그랬는지는 모르겠다. 아마도 처음에 어렵다고 느껴서 나 혼자서 이렇게 정의내렸던 것 같다.) _ = button.rx.tap.debug(\"TAP\") .flatMap { return Observable&lt;Int&gt;.error(TestError.fakeError).debug(\"내부 Observable\") }.debug(\"flatMap 이후\") .subscribe() //여기서는 스스로 dispose되는지 확인만 하면 되므로 굳이 disposeBag에는...","categories": ["RxSwift"],
        "tags": [],
        "url": "https://ictechgy.github.io/rxswift/flatMap/",
        "teaser": "https://ictechgy.github.io/assets/images/photo-hangang.jpeg"
      },{
        "title": "zip Operator에 대하여(RxSwift)",
        "excerpt":"zip은 대표적인 Observable 병합 연산자이다. 내가 가진 궁금증은 아래와 같다.   Observable.zip을 중첩하여 사용하는 것은 가능할까? 이를테면 아래와 같이 사용이 가능한 것일까? _ = Observable.zip( Observable.just(1).debug(\"중간결과1\"), Observable.zip(Observable.just(2), Observable.just(3)) { return $0 + $1 }.debug(\"중간결과2\") ) { return $0 + $1 }.debug(\"결과값\") .subscribe() 내부에 있는 Observable.zip에 대해서는 subscribe를 한 것도 아니기...","categories": ["RxSwift"],
        "tags": [],
        ...
        "excerpt":"Swift를 다시 공부하던 도중 기록하면 좋을만한 부분이 있어서 작성한다. Any?, AnyObject? 같은 옵셔널이 아닌 Any, AnyObject 등에는 nil을 직접적으로 넣을 수 없다. 하지만 아래와 같은 경우에는 가능하다. let a: Int? = nil let b: Any = a as Any //syntax error가 뜨지 않는다. 'Any?'로 쓰십시오!라고 뜰 줄 알았는데.. //또는 let...","categories": ["Swift","RxSwift"],
        "tags": [],
